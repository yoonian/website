...
- 클러스터의 스케일 축소를 위한
  노드 드레이닝([클러스터 오토스케일링](/ko/docs/tasks/administer-cluster/cluster-management/#cluster-autoscaler)에 대해 알아보기
  ).
...
만약 자발적인 중단을 일으킬 수 있는 원인이 없다면 파드디스룹션버짓(Pod Disruption Budget)의 생성을 넘길 수 있다.

{{< caution >}}
모든 자발적인 중단이 파드디스룹션버짓(Pod Disruption Budget)에 연관되는 것은 아니다.
예를 들어 디플로이먼트 또는 파드의 삭제는 파드디스룹션버짓(Pod Disruption Budget)를 무시한다.
{{< /caution >}}
...
- 복제된 애플리케이션의 구동 시 훨씬 더 높은 가용성을 위해 랙 전체
  ([안티-어피니티](/docs/user-guide/node-selection/#inter-pod-affinity-and-anti-affinity-beta-feature) 이용)
  또는 영역 간
...
불리는 파드디스룹션버짓(Pod Disruption Budget)을 준수하는 도구를 이용해야 한다.
...
파드디스룹션버짓(Pod Disruption Budget)은 역할 분리에 따라
역할에 맞는 인터페이스를 제공한다.

만약 조직에 역할 분리에 따른 책임의 분리가 없다면
파드디스룹션버짓(Pod Disruption Budget)을 사용할 필요가 없다.
...
* [파드디스룹션버짓(Pod Disruption Budget) 설정하기](/docs/tasks/run-application/configure-pdb/)의 단계를 따라서 애플리케이션을 보호한다.
